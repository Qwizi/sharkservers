...
@router.get("/{identity}")
async def get_admin(admin: Admin = Depends(get_valid_admin)) -> AdminOut:
    return admin
...
async def create_admin(
    ...
    if data.groups_id:
        for group_id in data.groups_id:
            group = await get_valid_group(
                group_id=group_id, group_service=groups_service
            )
            groups_list.append(group)
    new_admin = await admins_service._create(
        data, groups_list=groups_list if groups_list else None
    )
    return new_admin
...
async def update_admin(
    ...
    if updated_data.groups_id:
        for group_id in updated_data.groups_id:
            ...
            groups_list.append(group)
    updated_admin = await admins_service._update(
        admin,
        updated_data=updated_data,
        groups_list=groups_list if groups_list else None,
    )
    return updated_admin


@router.delete("/{identity}")
async def delete_admin(admin: Admin = Depends(get_valid_admin)):
    await admin.delete()
    return admin
