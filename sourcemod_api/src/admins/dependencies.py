...
async def get_valid_admin(
    identity: str, admin_service: AdminService = Depends(get_admins_service)
):
    try:
        return await admin_service.get_one(identity=identity, related=["groups"])
    except NoMatch:
...
async def get_valid_group(
    group_id: int, group_service: GroupService = Depends(get_groups_service)
):
    try:
        return await group_service.get_one(id=group_id)
    except NoMatch:
...
async def get_valid_group_ovrride(
    group_override_id: int,
    group_override_service: GroupOverrideService = Depends(
        get_groups_overrides_service
    ),
):
    try:
