...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["server"], ["servers.id"], name="fk_playerstats_servers_id_server"
        ),
        sa.ForeignKeyConstraint(
            ["steam_profile"],
            ["players.id"],
            name="fk_playerstats_players_id_steam_profile",
        ),
        sa.PrimaryKeyConstraint("id"),
    )
...
def downgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "steamid32", sa.VARCHAR(length=255), autoincrement=False, nullable=False
        ),
        sa.Column(
            "steamid64", sa.VARCHAR(length=255), autoincrement=False, nullable=False
        ),
        sa.Column(
            "profile_url", sa.VARCHAR(length=255), autoincrement=False, nullable=True
        ),
        sa.Column("avatar", sa.VARCHAR(length=255), autoincrement=False, nullable=True),
        sa.Column(
            "username", sa.VARCHAR(length=32), autoincrement=False, nullable=False
        ),
        sa.Column(
            "steamid3", sa.VARCHAR(length=255), autoincrement=False, nullable=False
        ),
        sa.Column(
            "country_code", sa.VARCHAR(length=15), autoincrement=False, nullable=False
        ),
        sa.ForeignKeyConstraint(
            ["user"], ["users.id"], name="fk_steamprofiles_users_id_user"
        ),
