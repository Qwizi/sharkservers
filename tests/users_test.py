...
async def test_get_staff_users(client):
    ...
    assert response.json()["items"][0]["user_display_role"][0][
        "username"
...
async def test_logged_user_change_username(logged_client):
    ...
    response = await logged_client.post(
        f"{USERS_ENDPOINT}/me/username", json={"username": new_username}
    )
    assert response.status_code == 200
...
async def test_logged_user_change_username_with_invalid_data(username, logged_client):
    response = await logged_client.post(
        f"{USERS_ENDPOINT}/me/username", json={"username": username}
    )
...
async def test_logged_user_change_username_with_already_taken_username(logged_client):
    response = await logged_client.post(
        f"{USERS_ENDPOINT}/me/username",
        json={"username": TEST_ADMIN_USER.get("username")},
    )
...
async def test_logged_user_change_display_role_when_role_not_exists_in_user_roles(
    ...
    response = await logged_client.post(
        f"{USERS_ENDPOINT}/me/display-role",
        json={"role_id": ProtectedDefaultRolesEnum.ADMIN.value},
    )
    assert response.status_code == 400
...
async def test_logged_user_upload_avatar_with_invalid_content_type(
    content_type, logged_client
):
    image_file, image_bytes = create_fake_image()
