...
from src.users.schemas import UserOut, UserOutWithEmail, ChangeUsernameSchema, ChangePasswordSchema, \
    ChangeDisplayRoleSchema, UserOut2Schema, CreateUserSchema
from src.users.services import users_service
from src.users.utils import _get_users, _change_user_username, _change_user_password, _change_user_display_role, \
...
async def get_user(user: User = Depends(valid_user_id)) -> UserOut:
    """
    Get user
    :param user:
    :param user_id:
