...
async def admin_get_scopes(
    ...
    user: User = Security(get_admin_user, scopes=["scopes:all"]),
...
async def admin_get_scope(
    scope_id: int, user: User = Security(get_admin_user, scopes=["scopes:retrieve"])
):
    dispatch(
        ScopesAdminEventsEnum.GET_ONE_PRE, payload={"data": scope_id, "user": user}
    )
...
async def admin_create_scope(
    ...
    user: User = Security(get_admin_user, scopes=["scopes:create"]),
) -> ScopeOut:
    """
    Admin create scope.
    :param scope_data:
    ...
    """
    dispatch(
        ScopesAdminEventsEnum.CREATE_PRE, payload={"data": scope_data, "user": user}
    )
